...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF
    {
        ...
        protected function _vueHTML($content)
        {
            ...
            $content = preg_replace('/<datamatrix([^>]*)>/isU', '<hr>datamatrix : $1<hr>', $content);
            $content = preg_replace('/<\/datamatrix([^>]*)>/isU', '', $content);
...
'.$content.'
         ...
         * @return boolean
         */
        protected function _tag_open_DATAMATRIX($param)
        {
            if ($this->_testIsDeprecated && (isset($param['size']) || isset($param['noborder'])))
                throw new HTML2PDF_exception(9, array('DATAMATRIX', 'size, noborder'));

            if ($this->_debugActif) $this->_DEBUG_add('DATAMATRIX');
            ...
            $this->parsingCss->save();
            $this->parsingCss->analyse('datamatrix', $param);
            $this->parsingCss->setPosition();
            $this->parsingCss->fontSet();
            ...
            if (!$this->_subPart && !$this->_isSubPart) {
                $this->pdf->write2DBarcode($param['value'], 'DATAMATRIX', $x, $y, $size, $size, $style);
            }
         ...
         * @return boolean
         */
        protected function _tag_close_DATAMATRIX($param)
        {
            // there is nothing to do here
